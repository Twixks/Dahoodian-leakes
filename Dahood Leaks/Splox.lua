...
local window = library:Window({name = "nanosync.hook"})

local legit = window:Tab({name = "Mousehack"})
local visuals = window:Tab({name = "Eyesight"})
--local eyesight = window:Tab({name = "Eyesight but argyyysync diffrent"})
local misc = window:Tab({name = "Diffrent"})
local settings = window:Tab({name = "Cogs"})

local visuals_test = visuals:Section({name = "Test"})

local settings_cheat = settings:Section({name = "Cheat"})

local misc_nanosync = misc:Section({name = "Argyyysync"})

settings_cheat:Button({name = "Unload", callback = function() window:Unload() end})

misc_nanosync:Toggle({name = "Network argyyysync", flag = "argyyysync_enabled"})
misc_nanosync:Slider({name = "Argyyysync delay", min = 0.1, max = 2, def = 0.2, dec = 10, flag = "argyyysync_delay"})
misc_nanosync:Slider({name = "Argyyysync ticks", min = 2, max = 20, def = 10, flag = "argyyysync_ticks"})

local esp_preview = {instances = {}}

do
    local preview_frame = utility:Draw("Square", v2new(520, 0), {
        Size = v2new(200, 250),
        Color = c3rgb(35, 35, 35),
        Parent = window.frame
    }, true)
    ...
    local plr_name = utility:Draw("Text", v2new(preview_frame.Size.X / 2, 2), {
        Font = 2,
        Size = 13,
        Color = Color3.new(1, 1, 1),
        Outline = true,
    ...
    local plr_box_outline = utility:Draw("Square", v2new(6, 17), {
        Size = preview_frame.Size - v2new(12, 33),
        Color = Color3.new(),
        Thickness = 3,
        Filled = false,
        Parent = preview_frame
    ...
    local plr_weapon = utility:Draw("Text", v2new(preview_frame.Size.X / 2, preview_frame.Size.Y - 15), {
        Font = 2,
        Size = 13,
        Color = Color3.new(1, 1, 1),
        Outline = true,
    ...
    local chams_head = utility:Draw("Square", v2new(preview_frame.Size.X / 2 - 20, 26), {
        Size = v2new(40, 35),
    ...
    local chams_torso = utility:Draw("Square", v2new(preview_frame.Size.X / 2 - 35, 63), {
        Size = v2new(70, 80),
        Color = Color3.new(255, 0, 0),
        Transparency = 0.4,
        Parent = preview_frame
    ...
    esp_preview.instances = {
        ...
        chams_head, chams_torso, chams_larm, chams_rarm, chams_lleg, chams_rleg
    }

    function esp_preview.change_accent(self, new_accent)
        for _, v in pairs(self.instances) do
            if tonumber(tostring(v.Transparency):sub(1, 3)) == 0.4 then
                v.Color = new_accent
            end
        end
    end
...
utility:Connect(uis.InputBegan, function(input)
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.Home then
        if window.frame.Visible and not window.fading then
            ...
            if window.sshit == "Eyesight" and not window.fading then
                for i, v in pairs(esp_preview.instances) do
                    v.Visible = true
                    v.Transparency = 0
...
utility:Connect(rs.RenderStepped, function(delta)
    for i, v in pairs(esp_preview.instances) do
        v.Visible = window.frame.Visible and window.sshit == "Eyesight"
    end
end)
...
utility:Connect(rs.Heartbeat, function()
    if lplr.Character and lplr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if library.flags["argyyysync_enabled"] then
            if argyyysync_stuff.do_ticks then
                sethiddenproperty(hrp, "NetworkIsSleeping", false)
                argyyysync_stuff.did_times += 1
    
                if argyyysync_stuff.did_times == library.flags["argyyysync_ticks"] then
                    argyyysync_stuff.did_times = 0
                    
                    argyyysync_stuff.do_ticks = false
                    argyyysync_stuff.rn_tick = tick()
                end
            else
            ...
            if tick()-argyyysync_stuff.rn_tick >= library.flags["argyyysync_delay"] then
                argyyysync_stuff.do_ticks = true
            end
        end
